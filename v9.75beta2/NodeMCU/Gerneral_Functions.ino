...
void Sync_Settings() {
  ...
  if ((Mower_Docked_Filter == 2) && (Mower_Running != 1)) {
      ...
      Sync_Settings_Now ++;
      SerialDebug.print("|Setup Sync:");
      SerialDebug.print(Sync_Settings_Now);
...
void Set_PCB_Wire_Status() {
    ...
    if (Auto_Wire == 1) {
      ...
      SerialDebug.print("|Wire Status:");
      if (Wire_Status_ON_OFF == 0) SerialDebug.print("OFF");
      if (Wire_Status_ON_OFF == 1) SerialDebug.print("ON");
      
      
    if ((Mower_Docked_Filter == 2) || (Charge_Detected == 1))   {
     SerialDebug.print("|Docked Counts Wire:");
     SerialDebug.print(Docked_Counts);

      // 20 continuous docked status feeds required to turn off wire
      if ( Docked_Counts >= 20) {
        SerialDebug.print("|Wire OFF due to docked");
        Wire_OFF();
        }
    ...
    if ((Mower_Running_Filter == 2) || (Tracking_Wire == 1)) {
      SerialDebug.println("Wire ON due to running");
      Wire_ON();
      Docked_Counts = 0;
      }
