...
void Receive_Set_Time_Data() {
  ...
  while (SerialMEGA.available() > 0) {
    ...
    if ( recieved != '\a' && recieved != '\b') {   
      ...
      Time_Date = Serial1_RX_Value.toInt();
      Serial1_RX_Value = "";
      }
      else if (recieved == '\f') {
      Time_Month = Serial1_RX_Value.toInt();
      Serial1_RX_Value = "";
      }
      else if (recieved == '\w') {
      Time_Year = Serial1_RX_Value.toInt();
      Serial1_RX_Value = "";
      }
      else if (recieved == '\h') {
      Time_Day = Serial1_RX_Value.toInt();
      Serial1_RX_Value = "";
      }
    else Serial.print(F("No Data Received|"));
  }
...
void Get_Initial_Values() {
    ...
    Menu_Active = 56;                           // Code to tell the Mower MEGA which data to transmit.
    Send_Menu_Selected_To_Mower_MEGA();         // Send the code to the Mower MEGA which data should be transfered

    Receive_7_Data_Sets();                      // Recieve data sets from the Mower MEGA
    Time_Year             = Data1;             // Fill the variable values with the data sets
    Time_Month            = Data2;
    Time_Date             = Data3;
    Time_Hour             = Data4;
    Time_Minute           = Data5;
...
void Receive_Docked_Data_RADEK() {
  ...
  int VoltsTX;
	...
	if (SerialComMEGA.update ())
	{
	  ...
	  if (recvBuff[0] == 10) {
		  VoltsTX             = recvBuff[1] | recvBuff[2] << 8;
		  Robot_Status_Value  = recvBuff[3];
		  Mower_Error_Value   = recvBuff[4];
		  Charging            = recvBuff[5];
		  GPS_Lock_OK_TX      = recvBuff[6];
		  Serial.print(F("|RecvDD:OK|"));
	  }
	}  
